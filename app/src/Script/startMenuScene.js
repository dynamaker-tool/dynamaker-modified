...
startMenuScene.prototype = {
	...
	refresh: function() {
		...
		if(typeof FileReader == 'undefined'){
			...
			//map browse
			ctx.fillText("谱面选择 (.dy/.xml)", windowWidth * 0.25, windowHeight * 0.41);
			if (mapFileCtrl) {
				ctx.textAlign = "right";
				ctx.fillStyle = "#0FF";
				if (mapFileCtrl.length == 4) {
					...
					switch (mapFileExName) {
						...
						default:
							mapFileType = false;
							ctx.fillStyle = "#F00";
							ctx.fillText(mapFileCtrl[0].name  + " 不可使用", windowWidth * 0.75, windowHeight * 0.45);
							mapFileType = false;
							break;
					}
					///////////////////
			...
			ctx.fillText("音频/视频选择 (.ogg/.wav/.mp3/.mp4)", windowWidth * 0.25, windowHeight * 0.5);
			if (musicFileCtrl) {
				ctx.textAlign = "right";
				ctx.fillStyle = "#0FF";
				...
				if (musicFileName.substr(-4, 4).toUpperCase() != ".WAV" && musicFileName.substr(-4, 4).toUpperCase() != ".OGG" && musicFileName.substr(-4, 4).toUpperCase() != ".MP3" && musicFileName.substr(-4, 4).toUpperCase() != ".MP4") {
					musicFileOk = false;
					ctx.fillStyle = "#F00";
					ctx.fillText(musicFileName  + " 不可使用", windowWidth * 0.75, windowHeight * 0.545);
				}
				else {
			...
			ctx.textAlign = "center";
			if (mapFileOk && (mapFileType == "DY" || musicFileOk)) {
				ctx.fillText("[ 编辑谱面 ]", windowWidth * 0.5, windowHeight * 0.62);
			}
			else if (musicFileOk){
				ctx.fillText("[ 创建新谱面 ]", windowWidth * 0.5, windowHeight * 0.62);
			}
			else {
				ctx.fillText("浏览并选择一个音频文件来制作新谱面", windowWidth * 0.5, windowHeight * 0.62);
			};
		...
		ctx.fillText("特别鸣谢：AXIS5, Syncable, Aenokoff 和 whitelava3203", windowWidth * 0.226, windowHeight * 0.05);
